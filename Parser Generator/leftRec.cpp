...
private:
    ...
    void leftFact() {
        ...
        for (auto x : productions) prods.push_back({x.first, x.second});
        productions.clear();
        int i = -1;
        while (++i < (int) prods.size()) {
            ...
            for (int j = 0; j < rhs.size(); j++) mp[rhs[j][0]].push_back(j);
            int f = -1;
            ...
            for (auto x : mp) {
                ...
                if (idx.size() == 1) {
                    ...
                    for (int k : idx) {
                        vector<string> rf;
                        for (int q = 1; q < rhs[k].size(); q++) rf.push_back(rhs[k][q]);
                        if (rf.empty()) rf.emplace_back("Epsilon");
                        st.emplace(rf);
                    }
